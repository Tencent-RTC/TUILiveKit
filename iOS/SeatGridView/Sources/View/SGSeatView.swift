...
class SGSeatView: UIView {
    ...
    let seatImageView: UIImageView = {
        let imageView = UIImageView(frame: .zero)
        imageView.image = SeatGridViewExtension<UIImage>.seatGridViewImage("seat_empty_icon")
        return imageView
    }()
    ...
    let muteImageView: UIImageView = {
        let imageView = UIImageView(frame: .zero)
        imageView.contentMode = .scaleAspectFit
        imageView.image = SeatGridViewExtension<UIImage>.seatGridViewImage("seat_audio_locked")
    ...
    let nameLabel: UILabel = {
        ...
        label.font = SeatGridViewExtension<UIFont>.customFont(ofSize: 14)
        label.textColor = .g9
    ...
    let ownerImageView: UIImageView = {
        let imageView = UIImageView(frame: .zero)
        imageView.image = SeatGridViewExtension<UIImage>.seatGridViewImage("seat_owner_icon")
        imageView.isHidden = true
        return imageView
    }()
    ...
    private func update(seatInfo: TUISeatInfo) {
        if let userId = seatInfo.userId, !userId.isEmpty {
            if let avatarUrl = seatInfo.avatarUrl, let url = URL(string: avatarUrl) {
                mainImageView.kf.setImage(with: url, placeholder: SeatGridViewExtension<UIImage>.avatarPlaceholderImage)
            } else {
                mainImageView.image = SeatGridViewExtension<UIImage>.avatarPlaceholderImage
            }
            nameLabel.text = seatInfo.userName ?? ""
            toUserOnSeatStyle()
        } else {
            if seatInfo.isLocked {
                seatImageView.image = SeatGridViewExtension<UIImage>.seatGridViewImage("seat_locked_icon")
            } else {
                seatImageView.image = SeatGridViewExtension<UIImage>.seatGridViewImage("seat_empty_icon")
            }
