...
struct KaraokeLyricsInfo: Equatable {
    var KaraokeLyricLineInfos: [KaraokeLyricsLineInfo]
}

struct KaraokeLyricsLineInfo: Equatable {
    var charStrArray: [KaraokeLyricsCharacterInfo]
    var startTime: Double
    var endTime: Double
}

struct KaraokeLyricsCharacterInfo: Equatable {
    let startTime: Int
    let duration: Int
    var endTime: Int {
...
public class KaraokeLyricParser: NSObject {
    static var isVtt: Bool = false
    static var lyricsLineInfo: KaraokeLyricsLineInfo = KaraokeLyricsLineInfo(charStrArray: [],
                                                                     startTime: 0,
                                                                     endTime: 0)

    static func parserLocalLyricFile(fileURL: URL) -> KaraokeLyricsInfo? {
        ...
        var lyricsInfo = KaraokeLyricsInfo(KaraokeLyricLineInfos: [])
        for str in lyricsArray {
                ...
                for lineStr in lyricsLineArray {
                    if lineStr.count > 0 {
                        ...
                        if isVtt {
                            ...
                            lyricsLineInfo.charStrArray.append(KaraokeLyricsCharacterInfo(startTime: startTime,
                                                                                      duration: duration,
            ...
            if !isVtt && lyricsLineInfo.charStrArray.count > 0 {
                let lineLyricModelTemp = KaraokeLyricsLineInfo(charStrArray: lyricsLineInfo.charStrArray,
                                                           startTime: lyricsLineInfo.startTime,
                                                           endTime: lyricsLineInfo.endTime)
                lyricsInfo.KaraokeLyricLineInfos.append(lineLyricModelTemp)
                lyricsLineInfo.charStrArray.removeAll()
                lyricsLineInfo.startTime = 0
            }
        }

        if lyricsInfo.KaraokeLyricLineInfos.count == 0 {
            return nil
        }
        return lyricsInfo
    }
