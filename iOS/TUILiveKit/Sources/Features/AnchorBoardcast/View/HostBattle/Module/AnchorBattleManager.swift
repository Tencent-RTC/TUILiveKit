...
typealias AnchorBattleStateUpdateClosure = (inout AnchorBattleState) -> Void

class AnchorBattleManager {
    var observableState = ObservableState<AnchorBattleState>(initialState: AnchorBattleState())
    var state: AnchorBattleState {
    ...
    init(context: AnchorManager.Context) {
        ...
        self.observableState = ObservableState(initialState: AnchorBattleState())
    }
    
    func subscribeState<Value>(_ stateSelector: StateSelector<AnchorBattleState, Value>) -> AnyPublisher<Value, Never> {
        return observableState.subscribe(stateSelector)
    }
    
    func resetState() {
        observableState.update { state in
            state = AnchorBattleState()
        }
    }
...
extension AnchorBattleManager {
    func onBattleStarted(battleInfo: TUIBattleInfo) {
        ...
        observableState.update { state in
            ...
            state.battleConfig = AnchorBattleConfig(battleInfo.config)
            state.durationCountDown = Int(battleInfo.config.duration)
        }
        startCountDown()
        
        var battleUsers = battleInfo.inviteeList.map { user in
            return AnchorBattleUser(user)
        }
        battleUsers.append(AnchorBattleUser(battleInfo.inviter))
    ...
    func onUserJoinBattle(battleId: String, battleUser: TUIBattleUser) {
        ...
        observableState.update { state in
            state.battleUsers.append(AnchorBattleUser(battleUser))
        }
    }
    ...
    func onBattleScoreChanged(battleId: String, battleUserList: [TUIBattleUser]) {
        sortedBattleUsersByScore(battleUsers: battleUserList.map { AnchorBattleUser($0) })
    }
    
    func onBattleRequestReceived(battleId: String, inviter: TUIBattleUser, invitee: TUIBattleUser) {
        observableState.update { state in
            state.battleId = battleId
            state.receivedBattleRequest = (battleId, AnchorBattleUser(inviter))
        }
    }
...
//            }
    ...
    func update(battleState: AnchorBattleStateUpdateClosure) {
        observableState.update(reduce: battleState)
    }
    
    private func sortedBattleUsersByScore(battleUsers: [AnchorBattleUser]) {
        ...
        observableState.update { state in
            ...
            var finalUsers: [AnchorBattleUser] = []
