...
protocol SGViewManagerDataProvider: NSObject {
    var seatListCount: Int { get }
}
...
class SGViewManager {
    ...
    private weak var provider: SGViewManagerDataProvider?
    
    init(provider: SGViewManagerDataProvider) {
...
extension SGViewManager {
    ...
    private func getBuiltInLayoutConfig(layoutMode: SGLayoutMode) -> SGSeatViewLayoutConfig? {
        guard let seatCount = provider?.seatListCount, seatCount > 0 else { return nil }
