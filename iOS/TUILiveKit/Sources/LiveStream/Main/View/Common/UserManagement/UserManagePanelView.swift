...
class UserManagePanelView: RTCBaseView {
    ...
    private let userManagePanelType: UserManagePanelType
    ...
    public init(user: TUIUserInfo, manager: LiveStreamManager, routerManager: LSRouterManager, coreView: LiveCoreView, type: UserManagePanelType) {
        self.user = user
    ...
    private func subscribeState() {
        ...
        let isAudioLockedPublisher = manager.subscribeState(StateSelector(keyPath: \LSMediaState.isAudioLocked))
        let isVideoLockedPublisher = manager.subscribeState(StateSelector(keyPath: \LSMediaState.isVideoLocked))
        let lockAudioUserListPublisher = manager.subscribeState(StateSelector(keyPath: \LSCoGuestState.lockAudioUserList))
        let lockVideoUserListPublisher = manager.subscribeState(StateSelector(keyPath: \LSCoGuestState.lockVideoUserList))
    ...
    private func generateFeatureClickPanelModel() -> LSFeatureClickPanelModel {
    ...
    private lazy var designConfig: LSFeatureItemDesignConfig = {
...
extension UserManagePanelView {
    ...
    private func disableChatClick(_ sender: LSFeatureItemButton) {
    ...
    private func kickOutOfRoomClick() {
        let alertInfo = LSAlertInfo(description: .localizedReplace(.kickOutAlertText, replace: user.userName.isEmpty ? user.userId : user.userName),
                                    imagePath: nil,
    ...
    private func muteSelfAudioClick(_ sender: LSFeatureItemButton) {
        if isSelfMuted {
    ...
    private func closeSelfCameraClick(_ sender: LSFeatureItemButton) {
        if isSelfCameraOpened {
            coreView?.stopCamera()
            sender.isSelected = !sender.isSelected
        } else {
    ...
    private func leaveSeatClick() {
        let alertInfo = LSAlertInfo(description: .leaveSeatAlertText, imagePath: nil,
                                    cancelButtonInfo: (.cancelText, .g3),
    ...
    private func kickOffSeatClick() {
        let alertInfo = LSAlertInfo(description: .localizedReplace(.hangupAlertText, replace: user.userName.isEmpty ? user.userId : user.userName),
                                    imagePath: nil,
                                    cancelButtonInfo: (.cancelText, .g3),
                                    defaultButtonInfo: (.disconnectText, .redColor)) { alertPanel in
            alertPanel.dismiss()
