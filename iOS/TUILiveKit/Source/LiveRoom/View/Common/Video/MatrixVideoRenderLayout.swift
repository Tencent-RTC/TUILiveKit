...
class MatrixVideoRenderLayout: UICollectionViewFlowLayout {
    private let renderUserList: Observable<[UserInfo]>
    ...
    init(renderUserList: Observable<[UserInfo]>) {
        self.renderUserList = renderUserList
        super.init()
    }
...
extension MatrixVideoRenderLayout {
    ...
    private func calculateEachCellFrame() {
        if renderUserList.value.count == 1 {
            fullScreenAttributes()
        } else if renderUserList.value.count == 3 {
            thirdAttributes()
        } else if renderUserList.value.count > 0 {
            divideAttributes()
        }
    }
    ...
    private func divideAttributes() {
        let cellCount = renderUserList.value.count
    ...
    private func getDivideColumn() -> Int {
        if renderUserList.value.count > 4 {
            return 3
        }
        return 2
    }
