...
class PrepareSettingPanel: UIView {
    private let store: LiveStore
    private let routerStore: RouterStore
    private var cancellableSet = Set<AnyCancellable>()
    private lazy var menus: [SwitchItem] = {
        var item = SwitchItem(title: .needRequestText, isOn: store.selectCurrent(RoomSelectors.getRoomSeatMode) == .applyToTake)
        item.action = { [weak self] isNeedToApply in
            guard let self = self else { return }
            self.store.dispatch(action: RoomActions.updateRoomSeatModeByAdmin(payload: isNeedToApply ? .applyToTake : .freeToTake))
        }
        let menus = [item]
        return menus
    }()
...
extension PrepareSettingPanel: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return menus.count
    }
}

extension PrepareSettingPanel: UITableViewDelegate {
