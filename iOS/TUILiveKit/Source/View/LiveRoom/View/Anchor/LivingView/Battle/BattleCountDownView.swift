...
class BattleCountDownView: UIView {
    ...
    private lazy var isInWaitingPublisher = store.select(BattleSelectors.getIsInWaiting)
    private var cancellableSet: Set<AnyCancellable> = []
    
    init(store: LiveStore, routerStore: RouterStore, countdownTime: TimeInterval) {
        self.store = store
    ...
    private lazy var backgroundView: BattleCountDownBackgroundView = {
        let view = BattleCountDownBackgroundView()
    ...
    @objc private func cancelButtonClick() {
        ...
        store.dispatch(action: BattleActions.cancelBattleRequest(payload: battleId))
    }
    ...
    private func subscribeBattleState() {
        var lastIsInWaiting: Bool = store.selectCurrent(BattleSelectors.getIsInWaiting)
        isInWaitingPublisher
            .receive(on: RunLoop.main)
            .sink { [weak self] isInWaiting in
                guard let self = self, lastIsInWaiting != isInWaiting else { return }
                lastIsInWaiting = isInWaiting
                if !isInWaiting {
                    self.dismissSelf()
                }
            }
