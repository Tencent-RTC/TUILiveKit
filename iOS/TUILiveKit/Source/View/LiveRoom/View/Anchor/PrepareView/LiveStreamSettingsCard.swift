...
class LiveStreamSettingsCard: UIView {
    ...
    private lazy var coverButtonView: UIButton = {
        ...
        view.kf.setImage(with: URL(string: store.roomState.coverURL), for: .normal, placeholder: UIImage.placeholderImage)
    ...
    private func subscribe() {
        ...
        roomCoverUrlPublisher
            ...
            .sink { [weak self] url in
                guard let self = self else { return }
                self.coverButtonView.kf.setImage(with: URL(string: self.store.roomState.coverURL),
                                            for: .normal, placeholder: UIImage.placeholderImage)
            }
            .store(in: &cancellableSet)
