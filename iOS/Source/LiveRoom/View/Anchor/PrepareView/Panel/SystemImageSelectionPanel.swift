...
class SystemImageSelectionPanel: UIView {
    ...
    init(engineService: RoomEngineService,configs:[SystemImageModel]) {
...
extension SystemImageSelectionPanel {
    ...
    func activateConstraints() {
        ...
        confirmButton.snp.remakeConstraints { make in
            make.centerX.equalToSuperview()
            make.bottom.equalToSuperview().inset(21.scale375Height())
...
extension SystemImageSelectionPanel {
    @objc func backButtonClick() {
        self.action?.value = .close
    }

    @objc func confirmButtonClick() {
        self.action?.value = .close
        liveRoomInfo.coverUrl.value = currentSelectModel?.url ?? liveRoomInfo.coverUrl.value
    }
}
...
extension SystemImageSelectionPanel:PopupPanelSubViewProtocol {
