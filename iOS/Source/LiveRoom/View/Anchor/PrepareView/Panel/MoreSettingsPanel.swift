...
class MoreSettingsModel {
    ...
    private(set) var enable: Observable<Bool>
    init(title: String, enable: Observable<Bool>) {
        self.title = title
        self.enable = enable
    }
}
...
class MoreSettingsCell: UITableViewCell {
    var item: MoreSettingsModel? {
        didSet {
            ...
            switchButton.isOn = item.enable.value
    ...
    @objc func switchButtonClick() {
        item?.enable.value = switchButton.isOn
    }
}
...
extension MoreSettingsPanel: PopupPanelSubViewProtocol {
