...
- (void)viewDidLoad {
    ...
    //加载背景图
    UIImage *backImage = [UIImage imageNamed:@"avatar0_100" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil];
    ...
    [_noOwnerTip setText:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.anchornotonline")];
...
- (void)initLogicView {
    if (!_logicView) {
        ...
        if (_btnLinkMic == nil) {
            ...
            [_btnLinkMic setImage: [UIImage imageNamed:@"linkmic_on" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            ...
            [_btnCamera setImage:[UIImage imageNamed:@"camera" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
...
- (void)initRoomLogic {
    ...
    [_liveRoom enterRoomWithRoomID:[_liveInfo.roomId intValue] callback:^(int code, NSString * error) {
        ...
        if (code == 0) {
            ...
            [self makeToast:error.length > 0 ? error : LiveRoomLocalize(@"Demo.TRTC.LiveRoom.enterroomfailed")];
...
- (void)startLinkMic {
    ...
    [_btnLinkMic setImage:[UIImage imageNamed:@"linkmic_off" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
    [_btnLinkMic setEnabled:NO];
    
    [self showWaitingNotice:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.waitforanchoraccept")];
    ...
    [self.liveRoom requestJoinAnchor:@"" responseCallback:^(BOOL agreed, NSString * reason) {
        ...
        if (agreed) {
            ...
            [self->_btnLinkMic setImage:[UIImage imageNamed:@"linkmic_off" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            ...
            self->_isWaitingResponse = NO;
            [self->_btnLinkMic setImage:[UIImage imageNamed:@"linkmic_on" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
            if ([reason length] > 0) {
...
- (void)stopLocalPreview {
    if (_isBeingLinkMic == YES) {
        ...
        //关闭本地摄像头，停止推流
        for (TCStatusInfoView* statusInfoView in _statusInfoViewArray) {
            if ([statusInfoView.userID isEqualToString:[TUILiveRoomProfileManager sharedManager].userId]) {
...
- (void)onWaitLinkMicResponseTimeOut {
    if (_isWaitingResponse == YES) {
        ...
        [_btnLinkMic setEnabled:YES];
        [self hideWaitingNotice];
        [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.micconnecttimeoutandanchornoresponse") parentView:self.view];
    }
}
...
- (void)showAlertWithTitle:(NSString *)title sureAction:(void(^)(void))callback {
    ...
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.confirm") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        if (callback) {
            callback();
        }
    }];
...
- (void)onRoomDestroy:(NSString *)roomID {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self showAlertWithTitle:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.anchorcloseinteraction") sureAction:^{
...
- (void)onError:(int)errCode errMsg:(NSString *)errMsg extraInfo:(NSDictionary *)extraInfo {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(errCode != 0){
            if (self->_isInVC) {
                __weak __typeof(self) weakSelf = self;
                [self showAlertWithTitle:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.anchorcloseinteractionroom") sureAction:^{
                    [weakSelf closeVCWithRefresh:YES popViewController:YES];
                }];
            }else{
...
- (void)onKickoutJoinAnchor {
    [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.sorryforkicked") parentView:self.view];
...
- (void)onAnchorEnter:(NSString *)userID {
    ...
    if ([userID isEqualToString:[TUILiveRoomProfileManager sharedManager].userId]) {
        return;
    }
...
- (BOOL)checkPlayUrl:(NSString *)playUrl {
    if (!([playUrl hasPrefix:@"http:"] || [playUrl hasPrefix:@"https:"] || [playUrl hasPrefix:@"rtmp:"] )) {
        [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.addressillegalandsupportrfhm") parentView:self.view];
        return NO;
    }
    if (_isLivePlay) {
        if ([playUrl hasPrefix:@"rtmp:"]) {
            ...
            [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.addressillegalandsupportrf") parentView:self.view];
            return NO;
        }
    } else {
        if ([playUrl hasPrefix:@"https:"] || [playUrl hasPrefix:@"http:"]) {
            if ([playUrl rangeOfString:@".flv"].length > 0) {
                ...
                [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.addressillegalandsupportfhm") parentView:self.view];
                return NO;
            }
    ...
    if (!_videoFinished) {
        if (_playType == PLAY_TYPE_VOD_FLV || _playType == PLAY_TYPE_VOD_HLS || _playType == PLAY_TYPE_VOD_MP4) {
            if (_videoPause) {
                ...
                [_logicView.playBtn setImage:[UIImage imageNamed:@"pause" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
                [[UIApplication sharedApplication] setIdleTimerDisabled:YES];
            } else {
                NSAssert(NO, @"");
                //                [self.liveRoom pause];
                [_logicView.playBtn setImage:[UIImage imageNamed:@"start" inBundle:LiveRoomBundle() compatibleWithTraitCollection:nil] forState:UIControlStateNormal];
                [[UIApplication sharedApplication] setIdleTimerDisabled:NO];
            }
...
- (void)onRecvGroupDeleteMsg {
    ...
    if (!_isErrorAlert) {
        _isErrorAlert = YES;
        __weak __typeof(self) weakSelf = self;
        [self showAlertWithTitle:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.endedinteractive") sureAction:^{
...
- (void)clickBtnLinkMic:(UIButton *)button {
    if (_isBeingLinkMic == NO) {
        ...
        if (statusAudio == AVAuthorizationStatusDenied) {
            [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.micauthorityfailed") parentView:self.view];
            return;
        }
        ...
        if (statusVideo == AVAuthorizationStatusDenied) {
            [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.cameraauthorityfailed") parentView:self.view];
            return;
        }
        
        if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8.0) {
            [TCUtil toastTip:LiveRoomLocalize(@"Demo.TRTC.LiveRoom.notsupporthardencodeandstartmicconnectfailed") parentView:self.view];
            return;
        }
...
- (void)makeToast:(NSString *)message {
    [[LiveRoomToastManager sharedManager] makeToastWithView:self.view message:message];
}
...
- (void)trtcLiveRoom:(TRTCLiveRoom *)trtcLiveRoom onAnchorEnter:(NSString *)userID {
    if (userID == self.liveInfo.ownerId) {
        self.isOwnerEnter = true;
        [self.liveRoom startPlayWithUserID:userID view:self.videoParentView callback:^(int code, NSString * _Nullable message) {
...
- (void)trtcLiveRoom:(TRTCLiveRoom *)trtcLiveRoom onRoomInfoChange:(TRTCLiveRoomInfo *)info {
    BOOL isCdnMode = [[NSUserDefaults standardUserDefaults] boolForKey:@"liveRoomConfig_useCDNFirst"];
    if (isCdnMode) {
        return;
    }
