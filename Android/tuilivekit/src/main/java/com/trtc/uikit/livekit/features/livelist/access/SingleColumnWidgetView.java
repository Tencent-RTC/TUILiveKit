package com.trtc.uikit.livekit.features.livelist.access;

import android.content.Context;
import android.text.TextUtils;
import android.util.AttributeSet;
import android.view.LayoutInflater;
import android.widget.FrameLayout;
import android.widget.ImageView;
import android.widget.TextView;

import androidx.annotation.NonNull;

import com.tencent.cloud.tuikit.engine.extension.TUILiveListManager;
import com.trtc.tuikit.common.imageloader.ImageLoader;
import com.trtc.uikit.livekit.R;

public class SingleColumnWidgetView extends FrameLayout {
    private final ImageView mImageAvatar;
    private final TextView  mTextRoomName;
    private final TextView  mTextAnchorName;

    public SingleColumnWidgetView(@NonNull Context context) {
        this(context, null, 0);
    }

    public SingleColumnWidgetView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SingleColumnWidgetView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        LayoutInflater.from(getContext()).inflate(R.layout.livelist_single_column_widget_item, this, true);
        mTextRoomName = findViewById(R.id.tv_room_name);
        mTextAnchorName = findViewById(R.id.tv_anchor_name);
        mImageAvatar = findViewById(R.id.iv_avatar);
    }

    public void init(TUILiveListManager.LiveInfo liveInfo) {
        updateLiveInfoView(liveInfo);
    }

    public void updateLiveInfoView(TUILiveListManager.LiveInfo liveInfo) {
        ImageLoader.load(getContext(), mImageAvatar, liveInfo.ownerAvatarUrl, R.drawable.livelist_default_avatar);
        mTextRoomName.setText(TextUtils.isEmpty(liveInfo.name) ? liveInfo.roomId : liveInfo.name);
        mTextAnchorName.setText(TextUtils.isEmpty(liveInfo.ownerName) ? liveInfo.ownerId : liveInfo.ownerName);
    }
}
