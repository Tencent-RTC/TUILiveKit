...
public class AnchorManagerDialog extends PopupDialog {
    ...
    public AnchorManagerDialog(@NonNull Context context, AudienceManager audienceManager,
    ...
    private boolean isSelfUser() {
        ...
        return mUserInfo.userId.equals(mAudienceManager.getCoreState().userState.selfInfo.getValue().userId);
    }

    private boolean isAdmin() {
        String selfUserId = mAudienceManager.getCoreState().userState.selfInfo.getValue().userId;
    ...
    private void addObserver() {
        mAudienceManager.getCoGuestState().lockAudioUserList.observeForever(lockAudioUserListObserver);
        mAudienceManager.getCoGuestState().lockVideoUserList.observeForever(lockVideoUserListObserver);
    ...
    private void removeObserver() {
        mAudienceManager.getCoGuestState().lockAudioUserList.removeObserver(lockAudioUserListObserver);
        mAudienceManager.getCoGuestState().lockVideoUserList.removeObserver(lockVideoUserListObserver);
    ...
    private void onMicrophoneButtonClicked() {
        ...
        if (isAdmin()) {
            mAudienceManager.getCoGuestManager().disableUserMediaDevice(mUserInfo.userId,
    ...
    private void updateMicrophoneButton() {
        boolean isAudioLocked =
                mAudienceManager.getCoGuestState().lockAudioUserList.getValue().contains(mUserInfo.userId);
        if (!isSelfUser()) {
            if (isAdmin()) {
    ...
    private void updateCameraButton() {
        boolean isVideoLocked =
                mAudienceManager.getCoGuestState().lockVideoUserList.getValue().contains(mUserInfo.userId);
        if (!isSelfUser()) {
            if (isAdmin()) {
