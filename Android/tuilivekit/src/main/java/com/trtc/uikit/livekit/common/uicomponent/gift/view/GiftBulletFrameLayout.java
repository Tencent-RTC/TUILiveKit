...
public class GiftBulletFrameLayout extends FrameLayout implements Handler.Callback {
    ...
    public boolean setGift(TUIGift gift, int giftCount, TUIGiftUser sender, TUIGiftUser receiver) {
        if (gift == null) {
            return false;
        }
        mGift = gift;
    ...
    private void initLayoutState() {
        if (mGift != null) {
            TUIImageLoader.loadImage(mContext, mImageGiftIcon, mGift.imageUrl, R.drawable.livekit_ic_avatar);
        }
        this.setVisibility(View.VISIBLE);
        if (mSender != null) {
            TUIImageLoader.loadImage(mContext, mImageSendUserIcon, mSender.avatarUrl,
