...
public class AnchorManagerDialog extends PopupDialog {
    ...
    public AnchorManagerDialog(@NonNull Context context, LiveStreamManager liveStreamManager,
    ...
    private boolean isSelfUser() {
        ...
        return mUserInfo.userId.equals(mLiveStreamManager.getUserState().selfInfo.userId);
    }

    private boolean isAdmin() {
        String selfUserId = mLiveStreamManager.getUserState().selfInfo.userId;
        if (TextUtils.isEmpty(selfUserId)) {
            return false;
        }
        return selfUserId.equals(mLiveStreamManager.getRoomState().ownerInfo.userId);
    }

    private void addObserver() {
        mLiveStreamManager.getCoGuestState().lockAudioUserList.observeForever(lockAudioUserListObserver);
        mLiveStreamManager.getCoGuestState().lockVideoUserList.observeForever(lockVideoUserListObserver);
    ...
    private void removeObserver() {
        mLiveStreamManager.getCoGuestState().lockAudioUserList.removeObserver(lockAudioUserListObserver);
        mLiveStreamManager.getCoGuestState().lockVideoUserList.removeObserver(lockVideoUserListObserver);
    ...
    private void onMicrophoneButtonClicked() {
        ...
        if (isAdmin()) {
            mLiveStreamManager.getCoGuestManager().disableUserMediaDevice(mUserInfo.userId,
    ...
    private void updateMicrophoneButton() {
        boolean isAudioLocked =
                mLiveStreamManager.getCoGuestState().lockAudioUserList.getValue().contains(mUserInfo.userId);
        if (!isSelfUser()) {
            if (isAdmin()) {
    ...
    private void updateCameraButton() {
        boolean isVideoLocked =
                mLiveStreamManager.getCoGuestState().lockVideoUserList.getValue().contains(mUserInfo.userId);
        if (!isSelfUser()) {
            if (isAdmin()) {
