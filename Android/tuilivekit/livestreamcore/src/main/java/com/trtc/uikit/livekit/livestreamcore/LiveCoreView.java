...
public class LiveCoreView extends FrameLayout {
    private static final String TAG = "LiveCoreView";
        ...
        mTUILiveListManager.startPreloadVideoStream(roomId, isMuteAudio, liveView.getTUIVideoView(),
                new TUIRoomDefine.PlayCallback() {
                    ...
                    public void onPlaying(String userId) {
                        Logger.info("LiveCoreView startPreloadVideoStream, userId:" + userId);
                        if (callback != null) {
                    ...
                    public void onPlayError(String userId, TUICommonDefine.Error error, String message) {
                        Logger.info("LiveCoreView startPreloadVideoStream error, error:" + error + ",message:" + message);
                        if (callback != null) {
    ...
    public void stopPreviewLiveStream(String roomId) {
        ...
        mTUILiveListManager.stopPreloadVideoStream(roomId);
        removeCoGuestLiveView(getMixUserId(roomId));
    }
    ...
    private void addLocalVideoView() {
        Logger.info(TAG + ":addLocalVideoView: selfUserId = " + mUserState.selfInfo.userId);
    ...
    private void removeLocalVideoView() {
        Logger.info(TAG + ":removeLocalVideoView: selfUserId = " + mUserState.selfInfo.userId);
    ...
    private void addCoGuestLiveView(UserInfo userInfo) {
        Logger.info(TAG + ":addCoGuestLiveView: userId = " + userInfo.userId);
        if (userInfo.userId.equals(mUserState.selfInfo.userId)) {
            Logger.info(TAG + ":addCoGuestLiveView: userId = selfUserId");
            return;
        }
    ...
    private void removeCoGuestLiveView(String userId) {
        Logger.info(TAG + ":removeCoGuestLiveView: userId = " + userId);
        if (userId.equals(mUserState.selfInfo.userId)) {
            Logger.info(TAG + ":removeCoGuestLiveView: userId = selfUserId");
            return;
        }
    ...
    private void addCoHostLiveView(ConnectionUser userInfo) {
        Logger.info(TAG + ":addCoHostLiveView: userId = " + userInfo.userId);
        if (userInfo.userId.equals(mUserState.selfInfo.userId)) {
            Logger.info(TAG + ":addCoHostLiveView: userId ==selfUserId");
            return;
        }
        LiveStreamView liveView = createRemoteLiveViewByUserId(userInfo.userId);
        mVideoLiveManager.getMediaManager().setRemoteVideoView(userInfo.userId,
    ...
    private void removeCoHostLiveView(ConnectionUser userInfo) {
        Logger.info(TAG + ":removeCoHostLiveView: userId = " + userInfo.userId);
        if (userInfo.userId.equals(mUserState.selfInfo.userId) || userInfo.userId.equals(mRoomState.ownerInfo.userId)) {
            Logger.info(TAG + ":removeCoHostLiveView: userId ==selfUserId");
            return;
        }
