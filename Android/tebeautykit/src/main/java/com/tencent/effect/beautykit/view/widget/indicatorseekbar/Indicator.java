...
public class Indicator {
    ...
    private void initIndicator() {
        if (mIndicatorType == IndicatorType.CUSTOM) {
            if (mIndicatorCustomView != null) {
                ...
                int progressTextViewId = mContext.getResources().getIdentifier(
                        "isb_progress", "id", mContext.getApplicationContext().getPackageName());
                if (progressTextViewId > 0) {
                    ...
                    if (view != null) {
                        if (view instanceof TextView) {
                            ...
                            throw new ClassCastException("the view identified by isb_progress "
                                    + "in indicator custom layout can not be cast to TextView");
                        }
                    }
                }
            } else {
                throw new IllegalArgumentException("the attr：indicator_custom_layout "
                        + "must be set while you set the indicator type to CUSTOM.");
            }
        } else {
            if (mIndicatorType == IndicatorType.CIRCULAR_BUBBLE) {
                mIndicatorView = new CircleBubbleView(
                        mContext, mIndicatorTextSize, mIndicatorTextColor, mIndicatorColor, "1000");
    ...
    private GradientDrawable getGradientDrawable() {
        ...
        if (mIndicatorType == IndicatorType.ROUNDED_RECTANGLE) {
            tvDrawable = (GradientDrawable)
                    mContext.getResources().getDrawable(R.drawable.te_beauty_isb_indicator_rounded_corners);
        } else {
            tvDrawable = (GradientDrawable)
                    mContext.getResources().getDrawable(R.drawable.te_beauty_isb_indicator_square_corners);
        }
    ...
    private void adjustArrow(float touchX) {
        ...
        if (indicatorScreenX + touchX < mIndicatorPopW.getContentView().getMeasuredWidth() / 2) {
            setMargin(mArrowView, -(int) (mIndicatorPopW.getContentView()
                    .getMeasuredWidth() / 2 - indicatorScreenX - touchX), -1, -1, -1);
        } else if (mWindowWidth - indicatorScreenX - touchX < mIndicatorPopW.getContentView().getMeasuredWidth() / 2) {
            setMargin(mArrowView, (int) (mIndicatorPopW.getContentView()
                    .getMeasuredWidth() / 2 - (mWindowWidth - indicatorScreenX - touchX)), -1, -1, -1);
        } else {
    ...
    private void setMargin(View view, int left, int top, int right, int bottom) {
        ...
        if (view.getLayoutParams() instanceof ViewGroup.MarginLayoutParams) {
            ...
            layoutParams.setMargins(left == -1
                    ? layoutParams.leftMargin : left, top == -1
                    ? layoutParams.topMargin : top, right == -1
                    ? layoutParams.rightMargin : right, bottom == -1
    ...
    void iniPop() {
        ...
        if (mIndicatorType != IndicatorType.NONE && mIndicatorView != null) {
            ...
            mIndicatorPopW = new PopupWindow(mIndicatorView, WindowManager.LayoutParams.WRAP_CONTENT,
    ...
    void update(float touchX) {
        ...
        if (mIndicatorPopW != null) {
            ...
            mIndicatorPopW.update(mSeekBar, (int) (touchX - mIndicatorPopW.getContentView().getMeasuredWidth() / 2),
                    -(mSeekBar.getMeasuredHeight() + mIndicatorPopW.getContentView().getMeasuredHeight()
                            - mSeekBar.getPaddingTop() /*- mSeekBar.getTextHeight() */ + mGap), -1, -1);
            adjustArrow(touchX);
        }
    }
    ...
    void show(float touchX) {
        ...
        if (mIndicatorPopW != null) {
            ...
            mIndicatorPopW.showAsDropDown(mSeekBar,
                    (int) (touchX - mIndicatorPopW.getContentView().getMeasuredWidth() / 2f),
                    -(mSeekBar.getMeasuredHeight() + mIndicatorPopW.getContentView().getMeasuredHeight()
                            - mSeekBar.getPaddingTop() /*- mSeekBar.getTextHeight()*/ + mGap));
            adjustArrow(touchX);
        }
    }
    ...
    /**
     * call this method to replace the current indicator with a new indicator view,
     * indicator arrow will be replace ,too.
     *
     * @param customIndicatorView a new content view for indicator.
     * @param progressTextView    this TextView will show the progress or tick text,
     *                            must be found in @param customIndicatorView
     */
