...
class SeatController extends Controller {
    ...
    TUIRequest request = liveService.takeSeat(
        ...
        TUIRequestCallback(
          ...
          onRejected: (String requestId, String userId, String message) {
            ...
            makeToast(msg: LiveKitLocalizations.of(Global.appContext())!.live_voiceroom_take_seat_rejected);
          },
          ...
          onTimeout: (String requestId, String userId) {
            seatState.mySeatApplicationId.value = "";
            viewState.linkStatus.value = LinkStatus.none;
            makeToast(msg: LiveKitLocalizations.of(Global.appContext())!.live_voiceroom_take_seat_timeout);
          },
          onError: (String requestId, String userId, TUIError error, String message) {
            seatState.mySeatApplicationId.value = "";
            viewState.linkStatus.value = LinkStatus.none;
            ErrorHandler.onError(error.value(), message);
          },
        ));
  ...
  Future<void> kickSeat(String userId) async {
    final result = await liveService.kickUserOffSeatByAdmin(0, userId);
    if (result.code != TUIError.success) {
  ...
  void onKickedOffSeat(seatIndex, TUIUserInfo operateUser) {
    makeToast(msg: LiveKitLocalizations.of(Global.appContext())!.live_voiceroom_kicked_out_of_seat);
  }
}
