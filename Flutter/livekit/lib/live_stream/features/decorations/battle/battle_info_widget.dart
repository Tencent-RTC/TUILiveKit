...
class BattleInfoWidget extends StatefulWidget {
  ...
  const BattleInfoWidget(
      {super.key, required this.liveStreamManager, required this.isOwner});
...
class _BattleInfoWidgetState extends State<BattleInfoWidget> {
  final ValueNotifier<bool> _startImageVisibilityNotifier =
      ValueNotifier(false);
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        final isBattleRunning =
            liveStreamManager.battleState.isBattleRunning.value;
        if (isBattleRunning &&
            widget.isOwner &&
            liveStreamManager.battleState.isShowingStartWidget) {
          ...
          child: Stack(
            ...
            children: [
              Visibility(
                  ...
                  child: _buildSingleBattleScoreWidget()),
              Visibility(
                  visible: isBattleRunning || isOnDisplayResult,
                  child: _buildBattleTimerWidget()),
              Visibility(
                  visible: isBattleRunning, child: _buildStartImageWidget()),
              Visibility(
                  visible:
                      liveStreamManager.battleState.isOnDisplayResult.value,
                  child: _buildResultImageWidget()),
            ],
          ),
        );
      },
  ...
  Widget _buildSingleBattleScoreWidget() {
    final battleUserMap = {
      for (var user in liveStreamManager.battleState.battleUsers.value)
        user.userId: user
    };

    final connectedBattleUsers = liveStreamManager
        .coHostState.connectedUsers.value
        .where((user) => battleUserMap.containsKey(user.userId))
        .map((user) => battleUserMap[user.userId]!)
        .toList();

    if (liveStreamManager.coHostState.connectedUsers.value.length != 2 ||
        connectedBattleUsers.length != 2) {
      return const SizedBox.shrink();
    }

    // 1v1 battle
    connectedBattleUsers.sort((a, b) => a.rect.left.compareTo(b.rect.left));
    return Positioned(
      top: 130.height,
      left: 0,
      right: 0,
      child: SingleBattleScoreWidget(
  ...
  Widget _buildBattleTimerWidget() {
    return ValueListenableBuilder(
      ...
      builder: (context, durationCount, _) {
        final isMultipleBattleMode =
            liveStreamManager.coreCoHostState.connectedUserList.value.length >=
                3;
        return Positioned(
            top: isMultipleBattleMode ? 132.height : 148.height,
            child: Stack(
              alignment: Alignment.topCenter,
              children: [
  ...
  Widget _buildResultImageWidget() {
    if (!widget.isOwner) {
      return Align(
        alignment: Alignment.topCenter,
        child: Padding(
          padding: EdgeInsets.only(top: 165.height),
          child: Image.asset(
            _getResultImageName(),
            package: Constants.pluginName,
            width: 234.width,
    ...
    final isAnchorInBattle =
        widget.liveStreamManager.battleState.battleUsers.value.any((user) =>
            user.userId ==
            widget.liveStreamManager.coreRoomState.ownerInfo.userId);
