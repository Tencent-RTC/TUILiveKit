...
class GiftManager {
  ...
  void _initReceiveGiftMessageListener() {
    ...
    isInit = true;
    listener = V2TimAdvancedMsgListener(onRecvNewMessage: _onReceiveGiftMessage);
  ...
  Future<bool> sendGift(GiftMessage message) async {
    ...
    if (message.sender == null || message.receiver == null || message.gift == null) {
    ...
    if (createCustomMessage.code == 0) {
      ...
      if (sendMessageRes.code == 0) {
        ...
        debugPrint("GiftManager sendBarrage fail,{code:${sendMessageRes.code}, desc:${sendMessageRes.desc}");
        return false;
      }
    } else {
  ...
  void getGiftData() async {
    ...
    String dataJson = await GiftCacheManager.getCachedJson(Constants.giftDataUrl);
    ...
    if (dataJson.isNotEmpty) {
      ...
      if (jsonMap.containsKey("giftList")) {
        List<GiftModel> giftList = (jsonMap['giftList'] as List).map((e) => GiftModel.fromJson(e)).toList();
  ...
  _onReceiveGiftMessage(V2TimMessage message) {
    debugPrint("GiftManager receiveBarrage onRecvNewMessage{msgID:${message.msgID},groupID:${message.groupID},"
    ...
    try {
      ...
      debugPrint("GiftManager GiftJson.fromJson jsonDecode Exception:${e.toString()}");
    }
  }
}
