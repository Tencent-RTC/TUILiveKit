...
import 'package:gift/widget/svga_player/painter.dart';
...
class SVGAAnimationController extends AnimationController {
  ...
  set movieEntity(MovieEntity? value) {
    ...
    if (value != null) {
      ...
      assert(movieParams.viewBoxWidth >= 0 && movieParams.viewBoxHeight >= 0 && movieParams.frames >= 1,
          "Invalid SVGA file!");
  ...
  TickerFuture forward({double? from}) {
    assert(_movieEntity != null, 'SVGAAnimationController.forward() called after dispose()?');
...
class _SVGAImageState extends State<SVGAImage> {
  ...
  void _handleChange() {
    if (mounted && !widget._controller._isDisposed && video != widget._controller.movieEntity) {
      setState(() {
        // rebuild
        video = widget._controller.movieEntity;
      });
